...
var dataTable = $('#dataTable').DataTable({
    ...
    columnDefs: [{
            ...
            render: function (data, type, row, meta) {
                ...
                html += '    <input type="checkbox" value="' + row.seq + '" id="sellList_checkbox_' + meta.row + '" name="checkKey" />';
                html += '    <label for="sellList_checkbox_' + meta.row + '">&nbsp;</label>';
                html += '</div>';
            ...
            selector: 'td',
            render: function (data, type, row, meta) {
                return row.sellDate.substring(0, 10);
            }
        }, {
            targets: 2,
            className: 'dt-body-center',
...
$(document).ready(function () {
    ...
    dataTable.settings()[0].oLanguage.sEmptyTable = '등록된 매출 내역이 없습니다.';
...
function removeSales() {
    ...
    if ($('input:checkbox[name=checkKey]:checked').length < 1) {
        swal({
            title: '선택된 매출 내역이 없습니다.',
            text: '삭제할 매출 내역을 선택해주세요.',
            icon: 'info',
            buttons: {
                confirm: {
                    text: '확인',
